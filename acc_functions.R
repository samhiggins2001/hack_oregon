...
htmlSummary<-function(sumset,
	...
	if(is.null(ptable)|!ncol(ptable)|!nrow(ptable)){
	...
	if(length(table_list)){
		for(i in 1:length(table_list)){
			...
			if( postfix %in% c("png","jpeg","bmp")){#if it's an image
				...
				toHTML(path_detail=path_detail, table_list=table_list[[i]], 
							 limit_col=limit_col,
...
SaveToHTML<-function(results,
	...
	for(n in selNames){
		...
		if(grepl(pattern="summary", x=n, ignore.case=T)){
			...
			print(n)
			# 			readline("enter to continue")
			toHTML(path_detail=path_detail, table_list=results$overlap_analysis, fname=outHTMLname, maxrows=10000, pagetitle=n)
