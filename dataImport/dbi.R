...
dbiWrite<-function(tabla, name="test_table",  appendToTable=F, dbname="interactome", port=5432, clean = T ){
	library("DBI")
	library("RPostgreSQL")
  ...
  #write the table
  res = try(expr=dbWriteTable(conn=con, name=name, row.names = 0, value=tabla, overwrite=!appendToTable, append=appendToTable), silent=T)
  if(length(res)){
...
safeWrite<-function(tab, tableName, dbname, port=5432, append=F){
	...
	goodRows = 1:nrow(tab)
	badRows = NULL
	res = try(expr=dbiWrite(tabla=tab, name=tableName, dbname=dbname, port=port, appendToTable=F), silent=T)
		...
		while(T){
				...
				if(grepl(pattern="line [0-9]+", x=res)){
					...
					if(badlineTmp!=badline){
						...
						res = try(expr=dbiWrite(tabla=tab[goodRows,], name=tableName, dbname=dbname, port=port, appendToTable=F), silent=T)
					} else {
